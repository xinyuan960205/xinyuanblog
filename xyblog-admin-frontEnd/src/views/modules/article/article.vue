  ...
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-row>
          <el-button v-for="tag in scope.row.tagList" :key="tag.id" size="mini">{{tag.tagName}}</el-button>
        </el-row>
      </template>
    </el-table-column>
    <el-table-column
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-switch
          ...
          active-color="#13ce66"
          @change="updateTop(scope.row.id,scope.row.isTop)">
        </el-switch>
      </template>
    </el-table-column>
    <el-table-column
      ...
      label="状态">
      <template slot-scope="scope">
        <el-tooltip class="item" effect="dark" content="点击发布" v-if="!scope.row.isPublish" placement="top">
          <el-button type="info" size="mini" @click="updatePublish(scope.row.id, true)">未发布</el-button>
        </el-tooltip>
        <el-tooltip class="item" effect="dark" content="点击下架" v-if="scope.row.isPublish" placement="top">
          <el-button type="success" size="mini" @click="updatePublish(scope.row.id, false)" v-if="scope.row.isPublish === true">已发布</el-button>
        </el-tooltip>
      </template>
    </el-table-column>
    <el-table-column
...
export default {
  ...
  methods: {
    ...
    updateRecommend (obj) {
      ...
      if(obj.isRecommend===true){
        this.addRecommend(data);
      }else{
        this.removeRecommend(obj.id);
      }
    ...
    addRecommend(dataForm){
      this.$http({
        url: this.$http.adornUrl(`/admin/operation/recommend/${!dataForm.id ? 'save' : 'update'}`),
    ...
    removeRecommend(id){
      var ids = [id]
      this.$http({
        url: this.$http.adornUrl('/admin/operation/recommend/deleteByArticleId'),
        method: 'delete',
