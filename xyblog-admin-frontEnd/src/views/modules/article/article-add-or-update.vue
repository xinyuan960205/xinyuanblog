  ...
  <div>
    <el-form :model="article" label-width="80px" :rules="rules" ref="articleForm">
      ...
      <el-form-item label="博文描述">
        <el-col :span="12">
          <el-input type="textarea" v-model="article.summary" placeholder="博文描述" clearable></el-input>
        </el-col>
      </el-form-item>
      ...
      selectValueList.forEach(value => {
        ...
        for (let i = 0; i < this.tagList.length; i++) {
          ...
          if (tag.id === value || value.id) {
            isInput = false
            tagList.push({id: tag.id, tagName: tag.tagName, type: 0})
          }
        }
        if (isInput) {
          tagList.push({tagName: value, type: 0})
        }
      })
