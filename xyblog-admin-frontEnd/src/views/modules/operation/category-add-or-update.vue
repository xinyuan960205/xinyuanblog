<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="120px">
      <el-form-item label="菜单级别" prop="categoryRank">
        <el-radio-group v-model="dataForm.categoryRank">
          <el-radio v-for="rank in rankList" :label="rank.parKey" :key="rank.parKey">{{ rank.parValue }}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item :label="getSysParam('CATEGORY_RANK',dataForm.categoryRank,rankList)+'名称'" prop="categoryName" >
...
export default {
  ...
  methods: {
        ...
        this.$http({
          ...
          this.$http({
            ...
            if (data && data.code === 200) {
              ...
              await this.categoryListTreeSetCurrentNode()
            } else {
              this.categoryList = []
            }
          })
    ...
    categoryListTreeSetCurrentNode () {
      ...
      this.dataForm.parentName = (this.$refs.categoryListTree.getCurrentNode() || {})['categoryName']
//      this.$refs.categoryListTree.setCheckedKeys(this.dataForm.parentId)
//      this.dataForm.parentName = (this.$refs.categoryListTree.getCheckedNodes() || {})['categoryName']
    },
  }
}
</script>
